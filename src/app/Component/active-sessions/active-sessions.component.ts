import { Component } from '@angular/core';
import { AuthService } from '../../Services/auth.service';
import { CommonModule } from '@angular/common';

@Component({
  selector: 'app-active-sessions',
  imports: [CommonModule],
  templateUrl: './active-sessions.component.html',
  styleUrl: './active-sessions.component.css'
})
export class ActiveSessionsComponent {
  ActiveSession: any[] = [];
  filteredUsers: any[] = [];

  constructor(private authService: AuthService) {}

  ngOnInit() {
    debugger;
    this.authService.getActivesession().subscribe(
      (data) => {
        console.log("API Response:", data);
        const arrayData = Array.isArray(data) ? data : [data];
        this.ActiveSession = arrayData;
        this.filteredUsers = this.ActiveSession;
      },
      (error) => {
        console.error("API Error:", error); 
      }
    );
  }

  applyFilter(event: Event) {
    const filterValue = (event.target as HTMLInputElement).value.toLowerCase();
    this.filteredUsers = this.ActiveSession.filter(user => 
      Object.values(user).some(v => {
        if (typeof v === 'string') {
          return v.toLowerCase().includes(filterValue);
        } else if (typeof v === 'number') {
          return v.toString().includes(filterValue);
        }
        return false;
      })
    );
  }
}
